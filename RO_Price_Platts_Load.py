...
FileList=[]
Tablename_Stg=GV.TDLogVar["DATABASE"] + ".DB250092_RO_PRICE_PLATTS_STG"
Tablename_Core=GV.TDLogVar["DATABASE"] + ".DB250092_RO_PRICE"

commit=10000
print("Reading Input Data")
input=GV.TDLogVar["DATADIR"] + "pricing/platts"
datalist=GV.ReadInputData(input,"*.ftp",2," ",commit)[0]            
rowcount=GV.ReadInputData(input,"*.ftp",2," ",commit)[1]            
if len(datalist)==0:
...
with con.cursor() as cur:
    ...
    sStageCreate = "CREATE MULTISET TABLE " + Tablename_Stg + ", \
        ...
        Price_DateTime VARCHAR(14) CHARACTER SET LATIN NOT CASESPECIFIC NOT NULL,\
        Price VARCHAR(20) CHARACTER SET LATIN NOT CASESPECIFIC\
        ) \
        PRIMARY INDEX (price_code,price_datetime)"
    try:
    ...
    try:
        sMerge="MERGE INTO " + Tablename_Core + " CORE USING (\
            ...
            Price_Code,Price_Freq,Price_Location,\
            Price_Description,NULL AS Price_Open,Price_Low,Price_High,\
            Price_Avg,NULL AS Price_Close,Price_Index,Price_Volume,\
            Price_UOM,Price_Country,Price_DeliveryType,Price_Currency \
