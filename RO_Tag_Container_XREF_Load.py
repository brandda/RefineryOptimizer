...
FileList=[]
Tablename_Stg=GV.TDLogVar["DATABASE"] + ".DB250092_RO_TAG_CONTAINER_XREF_STG"
Tablename_Core=GV.TDLogVar["DATABASE"] + ".DB250092_RO_TAG_CONTAINER_XREF"

commit=10000
print("Reading Input Data")
input=GV.TDLogVar["DATADIR"]
datalist=GV.ReadInputData(input,"Tag_Container_XREF.csv",1,",",commit)[0]            
rowcount=GV.ReadInputData(input,"Tag_Container_XREF.csv",1,",",commit)[1]            
if len(datalist)==0:
...
with con.cursor() as cur:
    ...
    try:
        sStageCreate="CREATE MULTISET TABLE " + Tablename_Stg + ", \
            NO BEFORE JOURNAL,NO AFTER JOURNAL,CHECKSUM = DEFAULT,DEFAULT MERGEBLOCKRATIO \
                ...
                Tag_ID VARCHAR(100) CHARACTER SET LATIN NOT CASESPECIFIC) \
    ...
    try:
        ...
        cur.execute(sTrunc)
        sReload="INSERT INTO " + Tablename_Core + " SELECT Ref_Location,Container_ID,Tag_ID,\
