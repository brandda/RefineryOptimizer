...
FileList=[]
Tablename_Stg=GV.TDLogVar["DATABASE"] + ".DB250092_RO_PRICE_CRACK_STG"
Tablename_Core=GV.TDLogVar["DATABASE"] + ".DB250092_RO_PRICE"

commit=10000
print("Reading Input Data")
input=GV.TDLogVar["DATADIR"]
datalist=GV.ReadInputData(input,"Crack.csv",2,",",commit)[0]            
rowcount=GV.ReadInputData(input,"Crack.csv",2,",",commit)[1]            
if len(datalist)==0:
...
with con.cursor() as cur:
    ...
    try:
        sStageCreate = "CREATE MULTISET TABLE " + Tablename_Stg + ", \
            ...
            (\
            WTI_Date VARCHAR(10) CHARACTER SET LATIN NOT CASESPECIFIC NOT NULL,\
            WTI_Price VARCHAR(20) CHARACTER SET LATIN NOT CASESPECIFIC,\
            Gasoline_Date VARCHAR(10) CHARACTER SET LATIN NOT CASESPECIFIC NOT NULL,\
            Gasoline_Price VARCHAR(20) CHARACTER SET LATIN NOT CASESPECIFIC,\
            Diesel_Date VARCHAR(10) CHARACTER SET LATIN NOT CASESPECIFIC NOT NULL,\
            Diesel_Price VARCHAR(20) CHARACTER SET LATIN NOT CASESPECIFIC\
            ) \
            NO PRIMARY INDEX"
    ...
    NullColumnList=["WTI_Price","Gasoline_Price","Diesel_Price"]
    ...
    try:
        sMerge="MERGE INTO " + Tablename_Core + " CORE USING (\
            SELECT Price_Source,Price_Type,Price_Date,Price_Time,\
                ...
                NULL AS Price_Open,NULL AS Price_Low,NULL AS Price_High,\
                NULL as Price_Avg,Price_Close,NULL as Price_Index,\
                NULL AS Price_Volume,Price_UOM,Price_Country,\
                NULL as Price_DeliveryType,Price_Currency \
