...
FileList=[]
Tablename_Stg=GV.TDLogVar["DATABASE"] + ".DB250092_RO_SENSOR_DESCUNITS_STG"
Tablename_Core=GV.TDLogVar["DATABASE"] + ".DB250092_RO_SENSOR_DESCUNITS"

commit=10000
print("Reading Input Data")
input=GV.TDLogVar["DATADIR"]
datalist=GV.ReadInputData(input,"sensor_descunits.csv",5,",",commit)[0]            
rowcount=GV.ReadInputData(input,"sensor_descunits.csv",5,",",commit)[1]            
if len(datalist)==0:
...
with con.cursor() as cur:
    ...
    try:
        sStageCreate="CREATE MULTISET TABLE " + Tablename_Stg + ", \
            ...
            Tag_ID VARCHAR(100) CHARACTER SET LATIN NOT CASESPECIFIC NOT NULL,\
            Tag_Desc VARCHAR(100) CHARACTER SET LATIN NOT CASESPECIFIC,\
            Tag_Units VARCHAR(100) CHARACTER SET LATIN NOT CASESPECIFIC) \
        PRIMARY INDEX (Ref_Location,Tag_ID)"
    ...
    NullColumnList=["Tag_Desc","Tag_Units"]
    ...
    try:
        ...
        cur.execute(sTrunc)
        sReload="INSERT INTO " + Tablename_Core + " SELECT Ref_Location,Tag_ID,Tag_Desc,\
            Tag_Units,'" + current_timestamp + "' FROM " + Tablename_Stg
        cur.execute(sReload)
