...
FileList=[]
Tablename_Stg=GV.TDLogVar["DATABASE"] + ".DB250092_RO_INVENTORY_DCD_STG"
Tablename_Core=GV.TDLogVar["DATABASE"] + ".DB250092_RO_INVENTORY_DCD"

commit=10000
print("Reading Input Data")
input=GV.TDLogVar["DATADIR"]
datalist=GV.ReadInputData(input,"Inventory_Dcd.csv",1,",",commit)[0]            
rowcount=GV.ReadInputData(input,"Inventory_Dcd.csv",1,",",commit)[1]            
if len(datalist)==0:
...
with con.cursor() as cur:
    ...
    try:
        sStageCreate="CREATE MULTISET TABLE " + Tablename_Stg + ", \
            ...
            Material VARCHAR(100) CHARACTER SET LATIN NOT CASESPECIFIC NOT NULL\
            ) \
        PRIMARY INDEX (Ref_Location,Material_ID,Material)"
...
#        print("       Null Update completed")
    ...
    try:
        ...
        cur.execute(sTrunc)
        sReload="INSERT INTO " + Tablename_Core + " SELECT Ref_Location,Material_ID,Material,\
